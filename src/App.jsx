            ...
            <Card>
              ...
              {[["Name", config.name], ["Ticker", `$${ticker}`], ["Country", config.country], ["Currency", `${curr}·${CURRENCIES[config.country]?.code}`], ["IPO Price", `${curr}${fmt(config.startPrice)}`], ["Date of Birth", config.dob || "—"]].map(([k, v]) => (
                <div key={k} className="flex justify-between items-center py-3 border-b border-[#111] last:border-0">
                  <span className="text-sm text-[#777]">{k}</span><span className="text-sm text-[#777]">{v}</span>
                </div>
              ))}
            </Card>
            <Card>
              ...
              <p className="text-xs text-[#777] mb-1">Full backup as JSON.</p>
              <p className="text-[10px] text-[#4a4a4a] mb-4">For Vercel: set <code className="text-[#666]">VITE_OPENROUTER_KEY</code> in env vars to override the hardcoded key.</p>
